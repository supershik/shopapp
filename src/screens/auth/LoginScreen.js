...
const LoginScreen = ({ navigation }) => {
    ...
    const requestPasswordReset = () => {
        ...
        setLoading(true);
        SHOPAPIKit.post('/shop/password/reset', payload)
            .then(onSuccess)
            .catch(onFailure);
    }
    ...
    const onPressForgotPassword = () => {
        ...
        const onSuccessMobile = ({ data }) => { //failed, mobile is unregistered
            setLoading(false);
        ...
        const onFailureMobile = error => {
            setLoading(false);
            if(error.toString().includes('409')) {  // success!, mobile already is registered
                setVerifyCode('');
                setVerificationId('');
    ...
    return (
        <View style={styles.container}>
            ...
            <ImageBackground style={styles.logoContainer} source={Logo} />
            <View style={{flex: 1, position: "absolute", width: '100%', alignItems: "center"}}>
